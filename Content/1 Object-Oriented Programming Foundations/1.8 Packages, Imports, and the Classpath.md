...
This is the first DIY in the repo's shared source tree. Production code lives under `src/main/java/` at the repo root — a legal-identifier path, unlike the lesson directories. Create two packages there, one for the domain class and one for a `Main` that uses it; compiled classes go to `out/main/`.
...
out/main/                    // created by javac -d out/main
```

Tasks:

1. **Compile and run from the classpath.** Build with `javac -d out/main $(find src/main/java -name '*.java')`, then run with `java -cp out/main app.Main`. Confirm it works.
2. **Break it deliberately and read the errors.** Try each of these in turn, observe the exact compiler or runtime message, then fix it:
   - Move `BankAccount.java` into the wrong directory (e.g., into `src/main/java/app/` while keeping `package banking;`). What does `javac` say?
   ...
   - Compile successfully, then run with `java -cp out/main/banking app.Main`. Why does it fail, and what error do you get?
   - Run with `java -cp out/main Main` (forgetting the package prefix). What error do you get?
...
- If you wanted to ship this program as a single JAR, what would the JAR's internal directory structure need to look like? (Hint: same shape as `out/main/`.)
...
- The course will soon add a `src/test/java/` tree alongside `src/main/java/` for JUnit. Given what you now know about the classpath, predict how the test command will need to differ from the production-code command. (Module 2.1 will confirm.)
